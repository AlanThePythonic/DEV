...
def run_cmd(ssh, cmd, indata=None):
    '''
...
# MAIN
# ================================================================
def connect_and_run(hostname, username, password, cmds ):
    ssh = MySSH()
    ...
    for cmd in cmds :
        if cmd.startswith("sudo") and sudo_pwd_used == False:
            run_cmd(ssh, cmd, password)
            sudo_pwd_used = True
        else :
            run_cmd(ssh, cmd)
    pass
def rebuild_spark_cluster() :
    '''
    steps:
    1. reboot all machines
    2. run start-master.sh in master
    3. run start-slave.sh spark://master-ip:7077 in all slaves
    '''
    username = "user"
    password = "111111"
    port = 22
    ...
    for hostname in spark_list :
        print "---- hostname:", hostname
        # Create the SSH connection
        connect_and_run(hostname, username, password, ["sudo reboot"])
         
    print "sleep 60 seconds..."
    time.sleep(60)
    # run start-master
    # there is no slaves defined in the conf/slaves file
    # and it would cause too much trouble to run start-master.sh command
    connect_and_run(master, username, password, \
        ["/bin/bash /Project/spark-1.6.1-bin-hadoop2.6/sbin/start-all.sh"])
    
    
    for hostname in slaves :
        connect_and_run(hostname, username, password, \
            ["/bin/bash /Project/spark-1.6.1-bin-hadoop2.6/sbin/start-slave.sh spark://%s:7077" % master])
    ...
    for hostname in spark_list :
        ...
        connect_and_run(hostname, username, password, \
            [
                ...
                "ls -lh /data/hadoopData | wc -l"
            ])
    

if __name__ == '__main__':
    rebuild_spark_cluster()
